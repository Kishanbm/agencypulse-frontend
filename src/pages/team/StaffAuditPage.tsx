...
import {
  ...
  Shield, FilterX, ArrowLeft, Activity
} from 'lucide-react';
...
type ChipId = 'all' | 'integrations' | 'reports';
...
export default function StaffAuditPage() {
  ...
  const { data, isLoading, isError } = useQuery<AuditLogResponse>({
    queryKey: ['auditLog', id, page, action, resourceType, appliedFrom, appliedTo],
  ...
  const { data: profile } = useQuery({
    queryKey: ['team', id],
    queryFn: () => api.get(`/team/${id}`).then((r) => r.data),
    enabled: !!id,
  });
      ...
      <div className="w-full max-w-5xl mx-auto space-y-6">
        ...
        >
          <div className="size-10 rounded-xl flex items-center justify-center" style={{ background: 'rgba(91,71,224,0.10)' }}>
            <Activity className="size-5" style={{ color: '#5B47E0' }} />
          </div>
          <div>
            <h1 className="font-heading font-bold text-xl sm:text-2xl tracking-tight text-foreground">
              {profile ? `${profile.firstName} ${profile.lastName}'s Activity` : 'Activity Log'}
            </h1>
            <p className="text-xs text-muted-foreground mt-0.5">A full history of actions performed by this user.</p>
          </div>
        </motion.div>
        ...
        >
          ...
          {isLoading && !data ? (
            ...
            <div className="py-16 text-center text-sm text-muted-foreground">
              Failed to load activity log. Please refresh.
            </div>
          ) : items.length === 0 ? (
            <div className="py-16 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground">No activity log entries match your filters.</p>
              {hasFilters && (
                <button
                  onClick={clearFilters}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead style={{ background: 'rgba(0,0,0,0.02)', borderBottom: '1px solid #ECECE6' }}>
                  <tr>
                    {['Timestamp', 'Action', 'Resource', 'Name', 'IP', 'Details'].map((h) => (
